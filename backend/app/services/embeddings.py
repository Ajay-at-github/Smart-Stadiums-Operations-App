"""Module containing Gemini batch embeddings client implementation."""
...
class GeminiBatchEmbeddings(Embeddings):
    """LangChain compatible embeddings class that calls Google Gemini's API.

    This client calls the embed_content API in batches and implements
    ...
    def _embed_with_retry(
        self, contents: Union[str, List[types.Content]]
    ) -> EmbedContentResponse:
        """Call the embedding API with exponential backoff.

        Handles transient and rate-limiting errors automatically.
