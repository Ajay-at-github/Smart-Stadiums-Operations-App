"""Main FastAPI application module defining routes, middleware, and CORS settings."""
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    Prevents DDoS attacks and Gemini API token exhaustion.
    """

    def __init__(
        self, app: FastAPI, limit: int = 60, window: int = 60
    ) -> None:
    ...
    async def dispatch(
        ...
        """Interpret and rate-limit requests targeting the chat route."""
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    """Middleware injecting core HTTP security response headers.

    Secures clients from common web vulnerabilities.
    """
...
async def chat(request: ChatRequest) -> ChatResponse:
    """Process incoming queries through the RAG matching.

    Utilizes context retrieval and Gemini text generation pipelines.
